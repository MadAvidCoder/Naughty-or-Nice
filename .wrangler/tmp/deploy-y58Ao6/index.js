...
var infractions = sqliteTable("infractions", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var appeals = sqliteTable("appeals", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  personId: integer("person_id").notNull().references(() => people.id, { onDelete: "cascade" }),
...
__name(getAllPeople, "getAllPeople");
function getPersonById(db, id) {
  return drizzle(db).select().from(people).where(eq(people.id, id)).limit(1).get();
}
__name(getPersonById, "getPersonById");
async function addPerson(db, name, is_nice, reason) {
  ...
  const row = await drizzle(db).select({ id: sql`last_insert_rowid()` }).from(people).get();
  if (!row) {
    return { id: -1 };
  }
  return { id: row.id };
}
__name(addPerson, "addPerson");
function judgePerson(db, id, isNice, reason) {
  drizzle(db).update(people).set({
    isNice,
    reason: reason || "",
...
__name(judgePerson, "judgePerson");
function deletePerson(db, id) {
  drizzle(db).delete(people).where(eq(people.id, id)).run();
  return { ok: true };
}
__name(deletePerson, "deletePerson");
function getInfractionsByPersonId(db, personId) {
  return drizzle(db).select().from(infractions).where(eq(infractions.personId, personId)).orderBy(desc(infractions.occurredAt)).all();
}
__name(getInfractionsByPersonId, "getInfractionsByPersonId");
async function addInfraction(db, personId, description, severity = 1) {
  ...
  const row = await drizzle(db).select({ id: sql`last_insert_rowid()` }).from(infractions).get();
  if (!row) {
    return { id: -1 };
  }
  return { id: row.id };
}
__name(addInfraction, "addInfraction");
async function createAppeal(db, personId, infractionId, appealText) {
  ...
  const row = await drizzle(db).select({ id: sql`last_insert_rowid()` }).from(appeals).get();
  if (!row) {
    return { id: -1 };
  }
  return { id: row.id };
}
__name(createAppeal, "createAppeal");
function listPendingAppeals(db) {
  return drizzle(db).select().from(appeals).where(eq(appeals.status, 0)).orderBy(desc(appeals.submittedAt)).all();
}
__name(listPendingAppeals, "listPendingAppeals");
function reviewAppeal(db, appealId, approved) {
  const new_status = approved ? 1 : 2;
  drizzle(db).update(appeals).set({ status: new_status }).where(eq(appeals.id, appealId)).run();
  return { ok: true };
}
__name(reviewAppeal, "reviewAppeal");
...
app.patch("/api/people/:id", async (c) => {
  ...
  const reason = body?.reason?.toString() ?? "";
  if (isNice === void 0) return c.json({ error: "isNice is required" }, 400);
...
app.get("/api/people/:id/infractions", async (c) => {
...
app.get("/api/appeals/pending", async (c) => {
...
app.patch("/api/appeals/:id/review", async (c) => {
  ...
  if (approved === void 0) return c.json({ error: "approved is required" }, 400);
